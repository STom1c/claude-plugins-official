...
const mcp = new Server(
  { name: 'line', version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      '',
      '## Google Calendar',
      'You have access to the user\'s Google Calendar via gcal_query.py. ALWAYS use it first for any calendar/schedule question.',
      '',
      'Run via Bash: cd /Users/stanley_tseng/py_projects && source .venv/bin/activate && python gcal_query.py <mode>',
      '',
      'Mode selection:',
      ...
      '',
      'Use search mode when: user mentions a course name, event name, or asks when something happens.',
      'Example: "Python課什麼時候?" → python gcal_query.py search Python課',
